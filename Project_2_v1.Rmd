...
Adding a nonlinear non-tree model to the lineup: support vector machine with a radial basis function (RBF) kernel via `kernlab`, wrapped by caret. SVM-radial is sensitive to scaling so it uses the narrow feature set with the same preProcess pipeline as the linear-family models (YeoJohnson, center, scale, knnImpute). Unlike trees, SVM doesn't natively output variable importance; caret falls back to a filter-based metric (LOESS pseudo-R-squared on each predictor).
...
- CV residuals are centered near zero (mean residual `r sprintf("%.5f", mean(cubist_preds$residual))`), so the model has almost no average bias. The negative value means it's slightly overpredicting PH on average, but the effect is tiny.
- Residual SD `r sprintf("%.4f", sd(cubist_preds$residual))`, in the same ballpark as the random forest residual spread. Most prediction errors are within roughly a tenth of a PH unit.
- Max absolute residual `r sprintf("%.4f", max(abs(cubist_preds$residual)))` with the 99th percentile at `r sprintf("%.4f", quantile(abs(cubist_preds$residual), 0.99))`. Most errors are small, with a handful of outlier misses that may be unusual production conditions or observations where the process variables don't fully explain PH.
- Predicted PH range `r sprintf("%.3f", min(cubist_preds$pred))` to `r sprintf("%.3f", max(cubist_preds$pred))` vs observed `r sprintf("%.3f", min(cubist_preds$obs))` to `r sprintf("%.3f", max(cubist_preds$obs))`. Cubist reaches further up than random forest did, getting closer to (or past) the observed maximum, but still doesn't quite hit the lowest observed values.
- Correlation between predicted PH and residuals is `r sprintf("%.4f", cor(cubist_preds$pred, cubist_preds$residual))`, close to zero. No strong systematic over/underprediction pattern at different PH levels. The small residual trend we saw in random forest is not present here.
...
All six models trained on the same shared CV folds. Comparing on identical resampling structure via `resamples()`:
...
- Six models trained: linear regression (lm), elastic net, random forest (RF), gradient boosting (gbm), support vector machine with radial kernel (SVM-radial), and Cubist. Ranked by cross-validated RMSE: Cubist (0.0926) ~= random forest (0.0926) < gradient boosting (0.107) < SVM-radial (0.123) < elastic net (0.135) ~= linear regression (0.135).
- **Cubist edges out random forest at the top.** Cubist: RMSE 0.0926, R-squared 0.7093, MAE 0.0666. RF: RMSE 0.0926, R-squared 0.7177, MAE 0.0672. Cubist wins on RMSE (by about 3e-5) and MAE (by about 1%); random forest wins on R-squared (by about 1%). Random forest was also a bit more stable in the per-fold RMSE distribution. The two are tied in practice; the auto-pick on lowest CV RMSE selects Cubist.
- Gradient boosting came in third (RMSE 0.107, R-squared 0.612). It did better than the linear models and SVM-radial, but didn't match the top two. The tuning landed at the largest settings we tested, so gbm might have kept improving with more tuning room, but the gap to the top was large enough that we didn't bother to chase it further.
- SVM-radial sits in between the linear and top tree/rule-based models (RMSE 0.123, R-squared 0.494). The radial kernel lets it fit curved relationships, which helps it do better than the linear models. But SVM-radial is a single model while random forest, gradient boosting, and Cubist each combine many sub-models into one prediction, which is probably the main reason those three outperform SVM-radial here.
...
- **Picking Cubist** as our final model. It won the auto-pick on lowest CV RMSE, its rule-based structure (rules with linear models inside each terminal node) is more transparent than RF's ensemble, and its predicted PH range tracks the observed range more closely than RF does. Random forest is essentially tied and remains a defensible alternative if anyone on the team prefers it for stability or familiarity.
...
- The chunk picks the best model dynamically by lowest CV RMSE in `model_metrics`. Right now that's `r best_model_name`.
...
1. **The model.** Cubist is our chosen final model with cross-validated RMSE 0.093, R-squared 0.71, MAE 0.067. Random forest came in essentially tied (RMSE 0.093, R-squared 0.72, MAE 0.067) and is a defensible alternative. Both are substantially better than the linear family (linear regression and elastic net both at RMSE 0.135, R-squared 0.38). Of the six models we tried, Cubist and random forest were the clear top two; gradient boosting came in third.
2. **The factors.** Consistent themes across all six models:
   - `Mnf.Flow`: the single most predictive variable across the lineup. Lower flow correlates with higher PH. Cubist puts it at the top; random forest has it tied at the top with Brand.Code C.
   - `Brand.Code`: a real driver in most models, though weighted differently. Random forest emphasizes Brand C the most (the brand with the lowest baseline PH). Linear models give the strongest weight to Brand D instead, with C and B also significant. Cubist gives Brand.Code less weight than the other models do, leaning more on continuous process variables. Either way, the brand a beverage is made under has a measurable effect on PH.
   - `Pressure.Vacuum`, `Alch.Rel`, `Balling.Lvl`, `Carb.Rel`: surface as important in the tree-based and rule-based models. These were dropped from the linear-family models due to collinearity, so they're "new" levers that pure linear analysis would have missed.
   - `Temperature`, `Usage.cont`, and `Oxygen.Filler`: appear at or near the top of every model.
3. **The recommendation.** Tighter operational tolerances and sensor monitoring on the top 4-5 predictors (or you could call them levers); each brand should get its own production targets since `Brand.Code` is consistently one of the top predictors of PH across most of the lineup (the right settings for Brand A probably aren't the right settings for Brand D). Specifically: focus on `Mnf.Flow` first (top driver in Cubist and tied at the top in random forest), then `Brand.Code`-aware standards, then the process variables `Pressure.Vacuum`, `Oxygen.Filler`, and `Alch.Rel`.

### Recap

- This is our working scratch pad for the executive doc. Cubist is our chosen final model (with random forest essentially tied as a defensible alternative) and the factors / recommendation bullets above are the current draft. We'll keep iterating on the phrasing as we polish for the team's non-technical report.
